...
- **代理群組**：手動選擇與自動測速（urltest）群組，可調測試網址、間隔、容差與大量節點並行數
...
- 支援遠端 / 直連 DNS 分流，包含 DoH、DoH3 與 DoT
- **Tailscale 私有組網**：MagicDNS、子網路路由與 Exit Node，可安全存取家中電腦、NAS 與私有服務
...
- [x] Tailscale / MagicDNS / Exit Node 與 DNS over HTTP/3
- [x] 多語系 UI（繁中 / 英文）
