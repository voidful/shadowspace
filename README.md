...
ShadowSpace 是一款 Shadowrocket 風格的 macOS 代理工具。它保留簡單的一鍵連線體驗，同時把公開直發版與本機上架工作區拆清楚：

| 路線 | 用途 | 引擎 | 狀態 |
|---|---|---|---|
| Developer ID 直發版 | 給需要完整協議、TUN、系統代理控制的使用者 | `sing-box（完整）`，也可切換 `原生` | 以公證 DMG 發佈 |
| Mac App Store 工作區 | 給沙箱環境與審查相容的透明代理版本 | `ShadowCore` + `NetworkExtension` | 本機 `AppStore/`，整包由 git ignore |
...
Mac App Store 工作區放在本機 `AppStore/` 內，整包由 `.gitignore` 排除。打包資源放在本機 `Resources/`，也不進公開 repo。這些資料可能包含送審筆記、截圖、metadata、簽章設定、佈建檔案、App Store Connect 資料、bundle plist、entitlements 與圖示素材。
...
- App Store 工作區可使用 `NETransparentProxyProvider`，以透明代理方式接流量
- 本機 App Store extension 已補 UDP flow 與 DNS 分流骨架；DNS 可依 Router policy 走 direct / proxy / reject
...
- App Store 工作區實機測試需要 Apple 核准的 Network Extensions capability 與 App Group 佈建描述檔
...
SwiftPM 無法直接產生 `.appex`，所以 App Store 版使用本機 `AppStore/` 工作區與產生器建立 Xcode 工程：
...
`AppStore/` 整包不進 git。若要公開分享上架流程，請把不含個資的摘要寫在頂層 README 或另外放到不含私有資料的文件。

## 常見問題

**直發版和 App Store 版差在哪？**  
直發版可使用完整 `sing-box` 能力，包含 TUN、系統代理設定與更多協議；App Store 工作區必須符合沙箱與審查規則，因此改用 `NetworkExtension` 和原生核心，功能範圍較保守。

**第一次連線很慢？**  
使用 `sing-box（完整）` 時，首次連線可能會下載核心引擎與分流規則檔；之後就會快很多。發佈版建議用 `make engine` 先內嵌核心。

**設定系統代理失敗？**  
修改網路設定需要管理員帳號。也可以關掉「連線時自動設定系統代理」，手動把 HTTP / SOCKS 代理指到 `127.0.0.1:7890`。App Store 工作區不使用這條路徑，而是由 NetworkExtension 接管。

**和 Shadowrocket 一樣有 VPN / TUN 模式嗎？**  
Developer ID 直發版的 `sing-box（完整）` 引擎支援 TUN / 增強模式。App Store 工作區使用 App Proxy Provider 透明代理，不包含 TUN 管理員模式。
...
- [x] DNS 自訂與訂閱自動更新
- [x] 本機 App Store NetworkExtension 工作區
...
Mac App Store 工作區改用 `ShadowCore` 與 Apple `NetworkExtension`，不包含 `sing-box`、外部核心下載或管理員授權流程；該工作區整包保留在本機，不進 git。
